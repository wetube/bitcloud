...
- Each storage node has a dedicated tunnel to an affiliated gateway. Normally,
  a storage node negotiate prices with the node grid owner, asking to become
  participant in the grid. The node interface should facilitate searching for
...
- Gateways can merge their authorities by signing each others' certificates,
  thus providing redundancy of operations in case of some gateways going down.
...
- Grids and publishers can make contracts. Several variables are in the
  contracts, like the mininum storage and minimum bandwidth rate to provide.
...
- Bitcloud checks QoS and releases or refund the contract depending on the
  statistics stored in the nodepool database.
- Publishers are in charge to register users. Each publisher must select the
  method he prefers to do so. For example, registering by mail, registering by
  introducing a captcha, etc. Publishers may generate anonymous users for read
...
Bitcloud introduces the concept of Proof of CA Generation, aimed to avoid
Sybil attacks.
It is imaginable that if a node or publisher can generate millions of CAs and
then try to register into the system, the whole Bitcloud would collapse trying
to allocate space in the database for them.

The way to avoid that is to impose the working on a hard to resolve problem
that takes hours or days in current computers, and irrevocably associate that
with the CA being registered. Any node or publisher unable to provide the
proof of work resolving the problem, will be rejected by the net.

When a new node is created, the interface advises the owner, asks the Bitcloud
for a problem, and resolves it. Once the network approves the node, it doesn't
need to repeat the process anymore, except if it fails to provide a minimum
average rate or 500Kps for the past 3 days to the public grid.
