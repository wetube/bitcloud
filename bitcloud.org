...
BitCloud is an effort to decentralize everything, including development and
thinking. This initiative is a progress toward to setting man free from men.

We welcome input and development from anyone who sees value in the protocol.
...
This repository is already long, and it is going to get much longer.
Some Key highlights of BitCloud:

1. The Bitcloud Cryptographic Law (BCL) is a series or set of "laws" for which there are
   "judges" comprised of net worker consonance serving to validate other workers and
...
2. The Bandwidth Law ensures that nodes route traffic from users
   to the Bitcloud net in order to get/post content. Nodes are compensated for doing so, by
   generating credit. Nodes connected to users are rewarded by the ammount of
   bandwidth they provide.
   https://github.com/wetube/bitcloud/blob/master/bitcloud.org#the-bitcloud-cryptography-law-bcl
   https://github.com/wetube/bitcloud/blob/master/bitcloud.org#bandwidth-law-proof-of-bandwidth
   
3. The Dispersion Law ensures that no one tries to subvert the system by
   dispersing the connections from user to nodes and other nodes or similar schemes.
   
   https://github.com/wetube/bitcloud/blob/master/bitcloud.org#dispersion-law

The priority is to determine the specifications and find any flaws by going
into the details. Major reestructuring could be feasible too, maintaining the
basic principles.

Are you ready to help us with your skills & knowledge? :)



* TO-IMPROVE Introduction

  Bitcloud is based on some of the ideas behind BitTorrent and Bitcoin technologies such as Mastercoin and
  Datacoin; it is meant to change the way we exchange digital information - as opposed to the current centralized trends.
